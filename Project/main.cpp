...
void GetAreaInfo(ifstream &fin,tArea &area){
    ...
    while(getline(fin, strTemp, '\n')){//while loop reads file til it finds the correct area heading
        ...
        if(strTemp==strArea){
            ...
            fin>>strTemp2>>area.strNorth;				
            fin>>strTemp2>>area.strEast;				
            fin>>strTemp2>>area.strSouth;				
            fin>>strTemp2>>area.strWest;//it then read in the surrounding area titles by skipping their <area> descriptors				
